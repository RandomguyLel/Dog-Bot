...
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name='Replit Test')
                               )
...
# Use regex to match a link starting with "https://reddit.com/"
  match = re.match(r'(https?:\/\/(?:www\.)?reddit\.com\/\S+)', message.content)

  if match:
    # Get the matched link
    link = match.group(1)
    author = message.author.name
    # Modify the link to replace "reddit" with "rxddit"
    modified_link = link.replace("reddit", "rxddit")
    # Extract the text part before and after the link
    #text_before_link = message.content.split(link)[0].strip()
    text_after_link = message.content.split(link)[-1].strip()
    # Send the modified link as a message
